...
// --- Additional conversion correspondence tests ---

TEST_CASE("Conversion: tautology A or notA closes", "[Conversion]")
{
    // Negate A ∨ ¬A → decompose ¬(A ∨ ¬A) → ¬A, A → ×
    TreeSerializer ser{};

    LogicalOr<> innerDisj{LogicalAtom{"A"}, *LogicalNot{LogicalAtom{"A"}}.Generalize()};
    LogicalNot<> negDisj{innerDisj};

    UnaryTreeNode root{negDisj};
    root.isPremise = true;

    auto nodeNegA = std::make_unique<UnaryTreeNode>(LogicalNot{LogicalAtom{"A"}});
    nodeNegA->antecedent = root.GetId();

    auto nodeA = std::make_unique<UnaryTreeNode>(LogicalAtom{"A"});
    nodeA->antecedent = root.GetId();

    auto closed = std::make_unique<ClosedTreeNode>();
    closed->decomposition1 = nodeA->GetId();
    closed->decomposition2 = nodeNegA->GetId();

    nodeA->SetChild(std::move(closed));
    nodeNegA->SetChild(std::move(nodeA));
    root.SetChild(std::move(nodeNegA));

    std::string json = ser.Serialize(root);
    REQUIRE(ser.serializedNodes.size() == 4);
    REQUIRE(json.find("\"text\":\"◯\"") == std::string::npos);
    REQUIRE(json.find("\"text\":\"×\"") != std::string::npos);
}

TEST_CASE("Conversion: neg conjunction branches negP negQ both open", "[Conversion]")
{
    // ¬(P ∧ Q) → branch ¬P | ¬Q, both open (satisfiable)
    TreeSerializer ser{};

    LogicalNot<> negConj{*LogicalAnd<LogicalAtom>{LogicalAtom{"P"}, LogicalAtom{"Q"}}.Generalize()};

    UnaryTreeNode root{negConj};
    root.isPremise = true;

    auto branch = std::make_unique<BinaryTreeNode>(root);

    auto leftNegP = std::make_unique<UnaryTreeNode>(LogicalNot{LogicalAtom{"P"}});
    leftNegP->antecedent = root.GetId();
    leftNegP->SetChild(std::make_unique<OpenTreeNode>());

    auto rightNegQ = std::make_unique<UnaryTreeNode>(LogicalNot{LogicalAtom{"Q"}});
    rightNegQ->antecedent = root.GetId();
    rightNegQ->SetChild(std::make_unique<OpenTreeNode>());

    branch->SetLeftChild(std::move(leftNegP));
    branch->SetRightChild(std::move(rightNegQ));
    root.SetChild(std::move(branch));

    std::string json = ser.Serialize(root);
    REQUIRE(ser.serializedNodes.size() == 6);

    size_t firstOpen = json.find("\"text\":\"◯\"");
    REQUIRE(firstOpen != std::string::npos);
    REQUIRE(json.find("\"text\":\"◯\"", firstOpen + 1) != std::string::npos);
}

TEST_CASE("Conversion: conditional branches negP or Q", "[Conversion]")
{
    // P → Q decomposes (when true) into ¬P | Q branch
    TreeSerializer ser{};

    LogicalConditional<LogicalAtom> cond{LogicalAtom{"P"}, LogicalAtom{"Q"}};

    BinaryTreeNode root{cond};
    root.isPremise = true;
    ...
    auto rightQ = std::make_unique<UnaryTreeNode>(LogicalAtom{"Q"});
    rightQ->antecedent = root.GetId();
    rightQ->SetChild(std::make_unique<OpenTreeNode>());

    root.SetLeftChild(std::move(leftNegP));
    root.SetRightChild(std::move(rightQ));

    std::string json = ser.Serialize(root);
    REQUIRE(ser.serializedNodes.size() == 5);
...
TEST_CASE("Conversion: negated implication linear P and negQ", "[Conversion]")
{
    // ¬(P → Q) decomposes linearly to P, ¬Q
    TreeSerializer ser{};

    LogicalConditional<LogicalAtom> cond{LogicalAtom{"P"}, LogicalAtom{"Q"}};
    LogicalNot<> negCond{*cond.Generalize()};

    UnaryTreeNode root{negCond};
    root.isPremise = true;

    auto nodeP = std::make_unique<UnaryTreeNode>(LogicalAtom{"P"});
    nodeP->antecedent = root.GetId();
    auto nodeNegQ = std::make_unique<UnaryTreeNode>(LogicalNot{LogicalAtom{"Q"}});
    nodeNegQ->antecedent = root.GetId();
    nodeNegQ->SetChild(std::make_unique<OpenTreeNode>());
    nodeP->SetChild(std::move(nodeNegQ));
    root.SetChild(std::move(nodeP));
